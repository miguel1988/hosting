...
if resp_dominio == None:
	...
	# Obtenemos el nombre del usuario al que pertenece el dominio introducido:
	...
	drop_bd = "drop database %s" %nombre_dominio
	cursor.execute(drop_bd)
	conexion_bd.commit()
	drop_user = "drop user %s@localhost" %nombre_my
	...
	print "El usuario %s y su dominio %s han sido borrado con exito"
